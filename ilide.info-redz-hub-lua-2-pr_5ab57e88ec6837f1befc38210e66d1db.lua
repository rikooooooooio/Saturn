...
local QuestTween = {}
local Sea1, Sea2, Sea3 = game.PlaceId == 2753915549, game.PlaceId == 4442272183,
...
local function PlayerTP(Tween_Pos)
  ...
  if (plrPP.Position - Tween_Pos.p).Magnitude > (Tween_Pos.p - PortalPos).Magnitude
+ 250 then
...
      TweenInfo.new(Distance / tonumber(getgenv().TweenSpeed * 1.8),
...
local function BoatTP(Boat, pos)
  if Boat then
    ...
    TweenInfo.new(Distance / getgenv().SeaBoatSpeed, Enum.EasingStyle.Linear,
...
local function KillAura()
  local function Kill(_,Enemie)
    ...
    if EnemieH and EnemieH.Health > 0 then
      ...
      if PlayerPP and EnemiePP and (EnemiePP.Position -
PlayerPP.Position).Magnitude < 1500 then
         EnemieH.Health = 0
  ...
  if pos then
    repeat task.wait()
      for _,v in pairs(pos) do
        ...
        if block then
           local tween = game:GetService("TweenService"):Create(block,
           TweenInfo.new((block.Position - v.p).Magnitude / getgenv().TweenSpeed,
Enum.EasingStyle.Linear),
...
   task.spawn(function()
     while task.wait() do
        pcall(function()
          ...
          if plrRP and Part and Part.Name == "HumanoidRootPart" and
Part.Parent:FindFirstChild("Humanoid") then
             local distance = math.floor((plrRP.Position - Part.Position).Magnitude /
3)
             local Health = math.floor(Part.Parent.Humanoid.Health)
             TL.Text = "Name : " .. Part.Parent.Name .. " | HP : " ..
tostring(Health).. " | MAG : " .. tostring(distance)
...
local function EspPlayer()
  while getgenv().EspPlayer do task.wait()
     for _,player in pairs(game:GetService("Players"):GetPlayers()) do
       if player.Name ~= Player.Name and player.Character and
...
local function EspFruits()
  while getgenv().EspFruits do task.wait()
     for _,Fruit in pairs(workspace:GetChildren()) do
       ...
       Fruit and string.find(Fruit.Name, "Fruit") and Fruit:FindFirstChild("Handle")
then
...
local function EspIslands()
  while getgenv().EspIslands do task.wait()
    local Locations = workspace:WaitForChild("_WorldOrigin",
...
local function EspChests()
  ...
  for _,Chest in pairs(workspace:GetChildren()) do
    if Chest then
      if Chest.Name == "Chest3" or Chest.Name == "Chest2" or Chest.Name == "Chest1"
then
        RemoveESP(Chest)
      end
    end
...
debug.getupvalues(require(Player.PlayerScripts.CombatFramework))
...
debug.getupvalues(require(Player.PlayerScripts.CombatFramework))
  ...
  for i = 1, 1 do
    local bladehit =
require(ReplicatedStorage.CombatFramework.RigLib).getBladeHits(Player.Character, {
    ...
    if #bladehit > 0 then
      pcall(function()
        ReplicatedStorage.RigControllerEvent:FireServer("weaponChange",
...
local function AttackDistance()
  while getgenv().AttackDistance do task.wait()
    pcall(function()
      local CF = debug.getupvalues(require(Player.PlayerScripts.CombatFramework))
[2]
...
local function PlayerClick()
  local plrPP = Player.Character and Player.Character.PrimaryPart
  if plrPP and TeleportPos and (plrPP.Position - TeleportPos).Magnitude > 25 then
return end
...
local function ActiveHaki()
  local VerifyHaki = Player.Character and
Player.Character:FindFirstChild("HasBuso")
  ...
  local function Verify(_,Enemie)
    if table.find(EnemiesList, Enemie.Name) then
      ...
      if EnemieH and EnemieH.Health > 0 then
        ...
        if EnemiePP and PlayerPP and (PlayerPP.Position -
EnemiePP.Position).Magnitude <= Distance then
...
task.spawn(function()
  ...
  oldhook = hookmetamethod(game, "__namecall", function(self, Vector, ...)
     if self.Name == "RemoteEvent" and getnamecallmethod():lower() == "fireserver"
then
...
local function FruitFind()
  ...
  for __,fruit in pairs(fruits) do
    ...
    local args3 = fruit and string.find(fruit.Name, "Fruit") and
fruit:FindFirstChild("Handle")

    if args1 and args2 and (args1.Position - args2.Position).Magnitude <=
FruitDistance then
      FruitDistance = (args1.Position - args2.Position).Magnitude
      Fruit = fruit
    elseif args1 and args3 and (args1.Position - args3.Position).Magnitude <=
FruitDistance then
...
local function AutoChestBypass()
  ...
  while getgenv().AutoChestBypass do task.wait()
    local plrRP = Player and Player.Character and
Player.Character:FindFirstChild("HumanoidRootPart")
    ...
    if not VerifyTool("God's Chalice") and not VerifyTool("Fist of Darkness") and
tick() - ActiveTick > 10 then
      local plrH = Player and Player.Character and
  ...
  while getgenv().AutoChestTween do task.wait()
    ...
    for _,ChestVerify in pairs(workspace:GetChildren()) do
      if ChestVerify:IsA("BasePart") and ChestVerify.Transparency < 1 then
        if ChestVerify.Name == "Chest3" or ChestVerify.Name == "Chest2" or
ChestVerify.Name == "Chest1" then
          if plrChar and (plrChar.Position - ChestVerify.Position).Magnitude <=
ChestDistance then
  ...
  if Sea1 then
    ...
    if Level < 10 then
      if tostring(Player.Team) == "Pirates" then
        ...
        QuestTween = {CFrame.new(943, 45, 1562), CFrame.new(1115, 45, 1619),
CFrame.new(1265, 45, 1606)}
    ...
    elseif Level >= 10 and Level < 15 then
      ...
      QuestTween = {CFrame.new(-1524, 50, 37), CFrame.new(-1424, 50, 216),
CFrame.new(-1554, 50, 359), CFrame.new(-1772, 50, 78), CFrame.new(-1715, 50, -61),
CFrame.new(-1594, 50, -45)}
    elseif Level >= 15 and Level < 30 then
      if VerifyNPC("The Gorilla King") and Level >= 20 then
        Quest = {CFrame.new(-1598, 37, 153), CFrame.new(-1128, 6, -451), "The
Gorilla King", "JungleQuest", 3}
        QuestTween = nil
      else
    ...
    elseif Level >= 40 and Level < 60 then
      if VerifyNPC("Bobby") and Level >= 55 then
        Quest = {CFrame.new(-1140, 4, 3829), CFrame.new(-1131, 14, 4080), "Bobby",
"BuggyQuest1", 3}
        QuestTween = nil
      else
...
CFrame.new(-1363, 55, 4162)}
    ...
    elseif Level >= 60 and Level < 75 then
      Quest = {CFrame.new(897, 6, 4389), CFrame.new(938, 6, 4470), "Desert Bandit",
"DesertQuest", 1}
       QuestTween = nil
    elseif Level >= 75 and Level < 90 then
       Quest = {CFrame.new(897, 6, 4389), CFrame.new(1546, 14, 4384), "Desert
Officer", "DesertQuest", 2}
       QuestTween = nil
    elseif Level >= 90 and Level < 100 then
       Quest = {CFrame.new(1385, 87, -1298), CFrame.new(1303, 106, -1441), "Snow
Bandit", "SnowQuest", 1}
...
CFrame.new(1228, 120, -1354)}
    elseif Level >= 100 and Level < 120 then
       if VerifyNPC("Yeti") and Level >= 105 then
         Quest = {CFrame.new(1385, 87, -1298), CFrame.new(1185, 106, -1518), "Yeti",
"SnowQuest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(1385, 87, -1298), CFrame.new(1185, 106, -1518),
"Snowman", "SnowQuest", 2}
         QuestTween = {CFrame.new(1243, 140, -1437), CFrame.new(1072, 140, -1458),
CFrame.new(1076, 140, -1621), CFrame.new(1209, 140, -1644), CFrame.new(1252, 140, -
1480)}
       end
    elseif Level >= 120 and Level < 150 then
       if VerifyNPC("Vice Admiral") and Level >= 130 then
         Quest = {CFrame.new(-5035, 29, 4326), CFrame.new(-4807, 21, 4360), "Vice
Admiral", "MarineQuest2", 2}
         QuestTween = nil
       else
         Quest = {CFrame.new(-5035, 29, 4326), CFrame.new(-4807, 21, 4360), "Chief
Petty Officer", "MarineQuest2", 1}
         QuestTween = nil
       end
    elseif Level >= 150 and Level < 175 then
       Quest = {CFrame.new(-4844, 718, -2621), CFrame.new(-4956, 296, -2901), "Sky
Bandit", "SkyQuest", 1}
       QuestTween = nil
    elseif Level >= 175 and Level < 190 then
       Quest = {CFrame.new(-4844, 718, -2621), CFrame.new(-5268, 392, -2213), "Dark
Master", "SkyQuest", 2}
       QuestTween = nil
    elseif Level >= 190 and Level < 210 then
       Quest = {CFrame.new(5306, 2, 477), CFrame.new(5288, 2, 470), "Prisoner",
"PrisonerQuest", 1}
       QuestTween = nil
    elseif Level >= 210 and Level < 250 then
       if VerifyNPC("Swan") and Level >= 240 then
         Quest = {CFrame.new(5191, 4, 692), CFrame.new(5230, 4, 749), "Swan",
"ImpelQuest", 3}
         QuestTween = nil
       elseif VerifyNPC("Chief Warden") and Level >= 230 then
         Quest = {CFrame.new(5191, 4, 692), CFrame.new(5230, 4, 749), "Chief
Warden", "ImpelQuest", 2}
         QuestTween = nil
       elseif VerifyNPC("Warden") and Level >= 220 then
         Quest = {CFrame.new(5191, 4, 692), CFrame.new(5230, 4, 749), "Warden",
"ImpelQuest", 1}
         QuestTween = nil
       else
         Quest = {CFrame.new(5306, 2, 477), CFrame.new(5282, 2, 1052), "Dangerous
Prisoner", "PrisonerQuest", 2}
        QuestTween = nil
      end
    elseif Level >= 250 and Level < 275 then
      Quest = {CFrame.new(-1581, 7, -2982), CFrame.new(-1897, 7, -2796), "Toga
Warrior", "ColosseumQuest", 1}
      QuestTween = nil
    elseif Level >= 275 and Level < 300 then
      Quest = {CFrame.new(-1581, 7, -2982), CFrame.new(-1327, 59, -3231),
"Gladiator", "ColosseumQuest", 2}
      QuestTween = {CFrame.new(-1268, 30, -2996), CFrame.new(-1472, 30, -3194),
CFrame.new(-1387, 30, -3438), CFrame.new(-1198, 30, -3508)}
    elseif Level >= 300 and Level < 325 then
      Quest = {CFrame.new(-5319, 12, 8515), false, "Military Soldier",
"MagmaQuest", 1}
    ...
    elseif Level >= 325 and Level < 375 then
      if VerifyNPC("Magma Admiral") and Level >= 350 then
        Quest = {CFrame.new(-5319, 12, 8515), CFrame.new(-5694, 18, 8735), "Magma
Admiral", "MagmaQuest", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(-5319, 12, 8515), CFrame.new(-5791, 97, 8834),
"Military Spy", "MagmaQuest", 2}
        QuestTween = nil
      end
    elseif Level >= 375 and Level < 400 then
      Quest = {CFrame.new(61122, 18, 1567), false, "Fishman Warrior",
"FishmanQuest", 1}
      QuestTween = {CFrame.new(60998, 50, 1534), CFrame.new(60880, 50, 1675),
CFrame.new(60785, 50, 1552), CFrame.new(60923, 60, 1262)}
    elseif Level >= 400 and Level < 450 then
      if VerifyNPC("Fishman Lord") and Level >= 425 then
        Quest = {CFrame.new(61122, 18, 1567), CFrame.new(61350, 31, 1095), "Fishman
Lord", "FishmanQuest", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(61122, 18, 1567), false, "Fishman Commando",
"FishmanQuest", 2}
        QuestTween = {CFrame.new(61866, 55, 1655), CFrame.new(62043, 55, 1510),
CFrame.new(61812, 55, 1254)}
      end
    elseif Level >= 450 and Level < 475 then
      Quest = {CFrame.new(-4720, 846, -1951), false, "God's Guard", "SkyExp1Quest",
1}
    ...
    elseif Level >= 475 and Level < 525 then
      if VerifyNPC("Wysper") and Level >= 500 then
        Quest = {CFrame.new(-7861, 5545, -381), CFrame.new(-7927, 5551, -637),
...
CFrame.new(-7643, 5580, -608)}
    ...
    elseif Level >= 525 and Level < 550 then
      Quest = {CFrame.new(-7903, 5636, -1412), false, "Royal Squad",
"SkyExp2Quest", 1}
       QuestTween = {CFrame.new(-7727, 5650, -1410), CFrame.new(-7522, 5650, -1499)}
     elseif Level >= 525 and Level < 625 then
       if VerifyNPC("Thunder God") and Level >= 575 then
         Quest = {CFrame.new(-7903, 5636, -1412), CFrame.new(-7751, 5607, -2315),
"Thunder God", "SkyExp2Quest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-7903, 5636, -1412), false, "Royal Soldier",
"SkyExp2Quest", 2}
         QuestTween = {CFrame.new(-7894, 5640, -1629), CFrame.new(-7678, 5640, -
1696), CFrame.new(-7672, 5640, -1903), CFrame.new(-7925, 5640, -1854)}
       end
     elseif Level >= 625 and Level < 650 then
       Quest = {CFrame.new(5258, 39, 4052), false, "Galley Pirate", "FountainQuest",
1}
       QuestTween = {CFrame.new(5391, 70, 4023), CFrame.new(5780, 70, 3969)}
     elseif Level >= 650 then
       if VerifyNPC("Cyborg") and Level >= 675 then
         Quest = {CFrame.new(5258, 39, 4052), CFrame.new(6138, 10, 3939), "Cyborg",
"FountainQuest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(5258, 39, 4052), false, "Galley Captain",
"FountainQuest", 2}
   ...
   elseif Sea2 then
     ...
     if Level >= 700 and Level < 725 then
       ...
       QuestTween = {CFrame.new(-614, 90, 2240), CFrame.new(-894, 90, 2275),
CFrame.new(-872, 90, 2481), CFrame.new(-552, 90, 2528)}
     elseif Level >= 725 and Level < 775 then
       if VerifyNPC("Diamond") and Level >= 750 then
         Quest = {CFrame.new(-427, 73, 1835), CFrame.new(-1569, 199, -31),
"Diamond", "Area1Quest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-427, 73, 1835), false, "Mercenary", "Area1Quest", 2}
         QuestTween = {CFrame.new(-867, 110, 1621), CFrame.new(-1047, 110, 1779),
CFrame.new(-1025, 110, 1087), CFrame.new(-1204, 110, 1195)}
       end
...
CFrame.new(1020, 110, 1366), CFrame.new(1016, 110, 1115)}
     elseif Level >= 800 and Level < 875 then
       if VerifyNPC("Jeremy") and Level >= 850 then
         Quest = {CFrame.new(635, 73, 919), CFrame.new(2316, 449, 787), "Jeremy",
"Area2Quest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(635, 73, 919), false, "Factory Staff", "Area2Quest", 2}
         QuestTween = {CFrame.new(882, 110, -49), CFrame.new(723, 110, 212),
CFrame.new(473, 110, 108), CFrame.new(-115, 110, 39)}
       end
     elseif Level >= 875 and Level < 900 then
       Quest = {CFrame.new(-2441, 73, -3219), false, "Marine Lieutenant",
"MarineQuest3", 1}
       QuestTween = {CFrame.new(-2552, 110, -3050), CFrame.new(-2860, 110, -3089),
CFrame.new(-3114, 110, -2947), CFrame.new(-2864, 110, -2679)}
    elseif Level >= 900 and Level < 950 then
       if VerifyNPC("Fajita") and Level >= 925 then
         Quest = {CFrame.new(-2441, 73, -3219), CFrame.new(-2086, 73, -4208),
"Fajita", "MarineQuest3", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-2441, 73, -3219), false, "Marine Captain",
"MarineQuest3", 2}
         QuestTween = {CFrame.new(-1695, 110, -3299), CFrame.new(-1953, 110, -3165),
CFrame.new(-2144, 110, -3341)}
       end
    elseif Level >= 950 and Level < 975 then
       Quest = {CFrame.new(-5495, 48, -794), false, "Zombie", "ZombieQuest", 1}
       QuestTween = {CFrame.new(-5715, 90, -917), CFrame.new(-5534, 90, -942),
CFrame.new(-5445, 90, -806), CFrame.new(-5485, 90, -663), CFrame.new(-5730, 90, -
590), CFrame.new(-5816, 90, -756)}
    elseif Level >= 975 and Level < 1000 then
       Quest = {CFrame.new(-5495, 48, -794), false, "Vampire", "ZombieQuest", 2}
       QuestTween = {CFrame.new(-6027, 50, -1130), CFrame.new(-6248, 50, -1281),
CFrame.new(-6120, 50, -1450), CFrame.new(-5951, 50, -1520), CFrame.new(-5803, 50, -
1360)}
    elseif Level >= 1000 and Level < 1050 then
       Quest = {CFrame.new(607, 401, -5371), false, "Snow Trooper",
"SnowMountainQuest", 1}
       QuestTween = {CFrame.new(445, 440, -5175), CFrame.new(523, 440, -5484),
CFrame.new(699, 440, -5612)}
    elseif Level >= 1050 and Level < 1100 then
       Quest = {CFrame.new(607, 401, -5371), false, "Winter Warrior",
"SnowMountainQuest", 2}
       QuestTween = {CFrame.new(1224, 460, -5332), CFrame.new(1404, 460, -5323),
CFrame.new(1258, 460, -5220), CFrame.new(1145, 460, -5077), CFrame.new(1022, 460, -
5139)}
    elseif Level >= 1100 and Level < 1125 then
       Quest = {CFrame.new(-6061, 16, -4904), false, "Lab Subordinate",
"IceSideQuest", 1}
       QuestTween = {CFrame.new(-5941, 50, -4322), CFrame.new(-5765, 50, -4304),
CFrame.new(-5608, 50, -4445), CFrame.new(-5683, 50, -4629)}
    elseif Level >= 1125 and Level < 1175 then
       if VerifyNPC("Smoke Admiral") and Level >= 1150 then
         Quest = {CFrame.new(-6061, 16, -4904), CFrame.new(-5078, 24, -5352), "Smoke
Admiral", "IceSideQuest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-6061, 16, -4904), false, "Horned Warrior",
"IceSideQuest", 2}
         QuestTween = {CFrame.new(-6306, 50, -5752), CFrame.new(-6161, 50, -5979),
CFrame.new(-6518, 50, -5795), CFrame.new(-6535, 50, -5640)}
       end
    elseif Level >= 1175 and Level < 1200 then
       Quest = {CFrame.new(-5430, 16, -5298), false, "Magma Ninja", "FireSideQuest",
1}
       QuestTween = {CFrame.new(-5233, 60, -6227), CFrame.new(-5194, 60, -6031),
CFrame.new(-5651, 60, -5854)}
    elseif Level >= 1200 and Level < 1250 then
       Quest = {CFrame.new(-5430, 16, -5298), false, "Lava Pirate", "FireSideQuest",
2}
       QuestTween = {CFrame.new(-4955, 60, -4836), CFrame.new(-5119, 60, -4634),
CFrame.new(-5389, 60, -4616), CFrame.new(-5342, 60, -4897)}
     elseif Level >= 1250 and Level < 1275 then
       Quest = {CFrame.new(1033, 125, 32909), false, "Ship Deckhand", "ShipQuest1",
1}
       QuestTween = {CFrame.new(1185, 180, 32979), CFrame.new(1204, 180, 33174),
CFrame.new(615, 180, 33169), CFrame.new(638, 180, 32962)}
     elseif Level >= 1275 and Level < 1300 then
       Quest = {CFrame.new(1033, 125, 32909), false, "Ship Engineer", "ShipQuest1",
2}
       QuestTween = {CFrame.new(809, 80, 33090), CFrame.new(751, 80, 32957),
CFrame.new(927, 80, 32724), CFrame.new(1040, 80, 32915), CFrame.new(1034, 80,
33058)}
     ...
     elseif Level >= 1325 and Level < 1350 then
       ...
       QuestTween = {CFrame.new(1238, 220, 33148), CFrame.new(1220, 220, 33426),
CFrame.new(622, 220, 33435), CFrame.new(593, 220, 33172)}
     elseif Level >= 1350 and Level < 1375 then
       Quest = {CFrame.new(5668, 28, -6484), false, "Arctic Warrior", "FrostQuest",
1}
       QuestTween = {CFrame.new(5836, 80, -6257), CFrame.new(6132, 80, -6098),
CFrame.new(6275, 80, -6237), CFrame.new(6095, 80, -6349)}
     elseif Level >= 1375 and Level < 1425 then
       if VerifyNPC("Awakened Ice Admiral") and Level >= 1400 then
         Quest = {CFrame.new(5668, 28, -6484), CFrame.new(6473, 297, -6944),
"Awakened Ice Admiral", "FrostQuest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(5668, 28, -6484), false, "Snow Lurker", "FrostQuest",
2}
         QuestTween = {CFrame.new(5700, 80, -6724), CFrame.new(5542, 80, -6898),
CFrame.new(5512, 80, -6641)}
       end
     elseif Level >= 1425 and Level < 1450 then
       Quest = {CFrame.new(-3056, 240, -10145), false, "Sea Soldier",
"ForgottenQuest", 1}
       QuestTween = {CFrame.new(-2583, 80, -9821), CFrame.new(-2830, 80, -9809),
CFrame.new(-3271, 80, -9729), CFrame.new(-3486, 80, -9813)}
     elseif Level >= 1450 then
       if VerifyNPC("Tide Keeper") and Level >= 1475 then
         Quest = {CFrame.new(-3056, 240, -10145), CFrame.new(-3711, 77, -11469),
"Tide Keeper", "ForgottenQuest", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-3056, 240, -10145), false, "Water Fighter",
"ForgottenQuest", 2}
         QuestTween = {CFrame.new(-3339, 290, -10412), CFrame.new(-3518, 290, -
10419), CFrame.new(-3536, 290, -10607), CFrame.new(-3345, 280, -10667)}
       end
     end
   elseif Sea3 then
     -- Auto Farm Level Sea 3
     if Level >= 1500 and Level < 1525 then
       Quest = {CFrame.new(-291, 44, 5580), false, "Pirate Millionaire",
"PiratePortQuest", 1}
       QuestTween = {CFrame.new(-44, 70, 5623), CFrame.new(-219, 70, 5546),
CFrame.new(-574, 70, 5496)}
    elseif Level >= 1525 and Level < 1575 then
      if VerifyNPC("Stone") and Level >= 1550 then
        Quest = {CFrame.new(-291, 44, 5580), CFrame.new(-1049, 40, 6791), "Stone",
"PiratePortQuest", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(-291, 44, 5580), false, "Pistol Billionaire",
"PiratePortQuest", 2}
        QuestTween = {CFrame.new(219, 105, 6018), CFrame.new(-24, 105, 6155),
CFrame.new(-312, 105, 6028)}
      end
    elseif Level >= 1575 and Level < 1600 then
      Quest = {CFrame.new(5834, 51, -1103), false, "Dragon Crew Warrior",
"AmazonQuest", 1}
      QuestTween = {CFrame.new(5992, 90, -1581), CFrame.new(6364, 90, -1512),
CFrame.new(6501, 90, -1104), CFrame.new(6612, 90, -938), CFrame.new(6393, 90, -
939)}
    elseif Level >= 1600 and Level < 1625 then
      Quest = {CFrame.new(5834, 51, -1103), false, "Dragon Crew Archer",
"AmazonQuest", 2}
      QuestTween = {CFrame.new(6472, 370, -151), CFrame.new(6547, 370, -51),
CFrame.new(6539, 410, 72), CFrame.new(6737, 410, 249), CFrame.new(6768, 410, 390),
CFrame.new(6625, 410, 371)}
    elseif Level >= 1625 and Level < 1650 then
      Quest = {CFrame.new(5448, 602, 748), false, "Female Islander",
"AmazonQuest2", 1}
      QuestTween = {CFrame.new(4836, 740, 928), CFrame.new(4708, 770, 911),
CFrame.new(4672, 790, 695), CFrame.new(4657, 800, 498), CFrame.new(4575, 810, 281)}
    elseif Level >= 1650 and Level < 1700 then
      if VerifyNPC("Island Empress") and Level >= 1675 then
        Quest = {CFrame.new(5448, 602, 748), CFrame.new(5730, 602, 199), "Island
Empress", "AmazonQuest2", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(5448, 602, 748), false, "Giant Islander",
"AmazonQuest2", 2}
        QuestTween = {CFrame.new(4784, 660, 155), CFrame.new(4662, 660, -57),
CFrame.new(4869, 660, -178), CFrame.new(5056, 660, -267), CFrame.new(5332, 660, -
166)}
      end
    elseif Level >= 1700 and Level < 1725 then
      Quest = {CFrame.new(2180, 29, -6738), false, "Marine Commodore",
"MarineTreeIsland", 1}
      QuestTween = {CFrame.new(3156, 120, -7837), CFrame.new(2904, 120, -7863),
CFrame.new(2606, 120, -7745), CFrame.new(2409, 120, -7874), CFrame.new(2269, 120, -
7416), CFrame.new(2413, 120, -7232), CFrame.new(2284, 120, -6911)}
    elseif Level >= 1725 and Level < 1775 then
      if VerifyNPC("Kilo Admiral") and Level >= 1750 then
        Quest = {CFrame.new(2180, 29, -6738), CFrame.new(2889, 424, -7233), "Kilo
Admiral", "MarineTreeIsland", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(2180, 29, -6738), false, "Marine Rear Admiral",
"MarineTreeIsland", 2}
        QuestTween = {CFrame.new(3205, 120, -6742), CFrame.new(3345, 120, -6649),
CFrame.new(3776, 210, -7254), CFrame.new(3879, 220, -7083), CFrame.new(3887, 210, -
6841), CFrame.new(3546, 210, -6809), CFrame.new(3448, 210, -7014), CFrame.new(3504,
210, -7230)}
       end
    elseif Level >= 1775 and Level < 1800 then
       Quest = {CFrame.new(-10581, 332, -8758), false, "Fishman Raider",
"DeepForestIsland3", 1}
       QuestTween = {CFrame.new(-10550, 380, -8574), CFrame.new(-10860, 380, -8459),
CFrame.new(-10578, 380, -8331), CFrame.new(-10377, 380, -8238), CFrame.new(-10147,
380, -8216), CFrame.new(-10234, 380, -8454)}
    elseif Level >= 1800 and Level < 1825 then
       Quest = {CFrame.new(-10581, 332, -8758), false, "Fishman Captain",
"DeepForestIsland3", 2}
       QuestTween = {CFrame.new(-10764, 380, -8799), CFrame.new(-10844, 380, -9030),
CFrame.new(-11160, 380, -9166), CFrame.new(-11073, 380, -8846), CFrame.new(-11043,
380, -8619)}
    elseif Level >= 1825 and Level < 1850 then
       Quest = {CFrame.new(-13233, 332, -7626), false, "Forest Pirate",
"DeepForestIsland", 1}
       QuestTween = {CFrame.new(-13335, 380, -7660), CFrame.new(-13138, 380, -7713),
CFrame.new(-13298, 380, -7876), CFrame.new(-13512, 380, -7983), CFrame.new(-13632,
380, -7815)}
    elseif Level >= 1850 and Level < 1900 then
       if VerifyNPC("Captain Elephant") and Level >= 1875 then
         Quest = {CFrame.new(-13233, 332, -7626), CFrame.new(-13393, 319, -8423),
"Captain Elephant", "DeepForestIsland", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-13233, 332, -7626), false, "Mythological Pirate",
"DeepForestIsland", 2}
         QuestTween = {CFrame.new(-13844, 520, -7016), CFrame.new(-13710, 520, -
6856), CFrame.new(-13482, 520, -6985), CFrame.new(-13224, 580, -6806)}
       end
    elseif Level >= 1900 and Level < 1925 then
       Quest = {CFrame.new(-12682, 391, -9901), false, "Jungle Pirate",
"DeepForestIsland2", 1}
       QuestTween = {CFrame.new(-12166, 380, -10375), CFrame.new(-12303, 380, -
10639), CFrame.new(-11904, 380, -10469), CFrame.new(-11636, 380, -10511),
CFrame.new(-11735, 380, -10687), CFrame.new(-11937, 380, -10713)}
    elseif Level >= 1925 and Level < 1975 then
       if VerifyNPC("Beautiful Pirate") and Level > 1950 then
         Quest = {CFrame.new(-12682, 391, -9901), CFrame.new(5241, 23, 129),
"Musketeer Pirate", "DeepForestIsland2", 3}
         QuestTween = nil
       else
         Quest = {CFrame.new(-12682, 391, -9901), false, "Musketeer Pirate",
"DeepForestIsland2", 2}
         QuestTween = {CFrame.new(-13098, 450, -9831), CFrame.new(-13222, 450, -
9621), CFrame.new(-13530, 450, -9760), CFrame.new(-13455, 450, -9940)}
       end
    elseif Level >= 1975 and Level < 2000 then
       Quest = {CFrame.new(-9481, 142, 5565), false, "Reborn Skeleton",
"HauntedQuest1", 1}
       QuestTween = {CFrame.new(-8680, 190, 5852), CFrame.new(-8879, 190, 5900),
CFrame.new(-8865, 190, 6100), CFrame.new(-8774, 170, 6169), CFrame.new(-8649, 190,
6071)}
    elseif Level >= 200 and Level < 2025 then
       Quest = {CFrame.new(-9481, 142, 5565), false, "Living Zombie",
"HauntedQuest1", 2}
       QuestTween = {CFrame.new(-10104, 200, 5739), CFrame.new(-10078, 200, 5953),
CFrame.new(-10195, 200, 6139), CFrame.new(-10252, 200, 5973)}
    elseif Level >= 2025 and Level < 2050 then
      Quest = {CFrame.new(-9515, 172, 6078), false, "Demonic Soul",
"HauntedQuest2", 1}
      QuestTween = {CFrame.new(-9275, 210, 6166), CFrame.new(-9379, 210, 6076),
CFrame.new(-9565, 210, 6201), CFrame.new(-9671, 210, 6096)}
    elseif Level >= 2050 and Level < 2075 then
      Quest = {CFrame.new(-9515, 172, 6078), false, "Posessed Mummy",
"HauntedQuest2", 2}
      QuestTween = {CFrame.new(-9442, 60, 6304), CFrame.new(-9427, 60, 6148),
CFrame.new(-9598, 60, 6121), CFrame.new(-9733, 60, 6119), CFrame.new(-9735, 60,
6336), CFrame.new(-9618, 60, 6323)}
    elseif Level >= 2075 and Level < 2100 then
      Quest = {CFrame.new(-2104, 38, -10194), false, "Peanut Scout",
"NutsIslandQuest", 1}
      QuestTween = {CFrame.new(-1870, 100, -10225), CFrame.new(-2143, 100, -10106)}
    elseif Level >= 2100 and Level < 2125 then
      Quest = {CFrame.new(-2104, 38, -10194), false, "Peanut President",
"NutsIslandQuest", 2}
      QuestTween = {CFrame.new(-2005, 100, -10585), CFrame.new(-2293, 88, -10512)}
    elseif Level >= 2125 and Level < 2150 then
      Quest = {CFrame.new(-818, 66, -10964), false, "Ice Cream Chef",
...
CFrame.new(-990, 100, -11085)}
    elseif Level >= 2150 and Level < 2200 then
      if VerifyNPC("Cake Queen") and Level >= 2175 then
        Quest = {CFrame.new(-818, 66, -10964), CFrame.new(-710, 382, -11150), "Cake
Queen", "IceCreamIslandQuest", 3}
        QuestTween = nil
      else
        Quest = {CFrame.new(-818, 66, -10964), false, "Ice Cream Commander",
...
CFrame.new(-720, 180, -11162)}
    ...
    elseif Level >= 2200 and Level < 2225 then
      Quest = {CFrame.new(-2023, 38, -12028), false, "Cookie Crafter",
"CakeQuest1", 1}
      QuestTween = {CFrame.new(-2332, 90, -12049), CFrame.new(-2468, 90, -12121),
CFrame.new(-2401, 90, -12224), CFrame.new(-2296, 90, -12114)}
...
CFrame.new(-1674, 90, -12206), CFrame.new(-1707, 90, -12360)}
    elseif Level >= 2250 and Level < 2275 then
      Quest = {CFrame.new(-1931, 38, -12840), false, "Baking Staff", "CakeQuest2",
1}
      QuestTween = {CFrame.new(-1930, 90, -12963), CFrame.new(-1803, 90, -13115),
CFrame.new(-1769, 90, -12955), CFrame.new(-1873, 90, -12755)}
...
CFrame.new(-2317, 110, -12994), CFrame.new(-2140, 110, -12989)}
    elseif Level >= 2300 and Level < 2325 then
      Quest = {CFrame.new(235, 25, -12199), false, "Cocoa Warrior", "ChocQuest1",
1}
      QuestTween = {CFrame.new(110, 80, -12245), CFrame.new(-71, 80, -12292)}
    elseif Level >= 2325 and Level < 2350 then
      Quest = {CFrame.new(235, 25, -12199), false, "Chocolate Bar Battler",
"ChocQuest1", 2}
       QuestTween = {CFrame.new(579, 80, -12413), CFrame.new(735, 80, -12659)}
     ...
     elseif Level >= 2400 and Level < 2425 then
       Quest = {CFrame.new(-1148, 14, -14446), false, "Candy Pirate", "CandyQuest1",
1}
       QuestTween = {CFrame.new(-1371, 70, -14405), CFrame.new(-1318, 70, -14715)}
     elseif Level >= 2425 and Level < 2450 then
       Quest = {CFrame.new(-1148, 14, -14446), false, "Snow Demon", "CandyQuest1",
2}
...
CFrame.new(-16912, 45, -152)}
     elseif Level >= 2500 and Level < 2525 then
       Quest = {CFrame.new(-16540, 56, 1051), false, "Sun-kissed Warrior",
"TikiQuest2", 1}
       QuestTween = {CFrame.new(-16345, 80, 1004), CFrame.new(-16425, 77, 1059),
CFrame.new(-16069, 37, 1029)}
     elseif Level >= 2525 then
       Quest = {CFrame.new(-16540, 56, 1051), false, "Isle Champion", "TikiQuest2",
2}
...
task.spawn(function()
  local function VerifyPos(_,Enemie)
    if Enemie.Name ~= "rip_indra True Form" then
      ...
      if EnemieRP and (EnemieRP.Position - Vector3.new(-5556, 314, -
2988)).Magnitude < 700 then
        PirateTime = tick()
      end
    end
  end
...
local function GetButton()
  ...
  if Circle then
    for _,part in pairs(Circle:GetChildren()) do
      if part and part.Name == "Part" and part:FindFirstChild("Part") and
part.Part.BrickColor ~= BrickColor.new("Lime green") then
        return part.Part
      end
    end
  end
...
local TimerG = Player:WaitForChild("PlayerGui", 9e9):WaitForChild("Main",
9e9):WaitForChild("Timer", 9e9)
local function Configure(Farm)
  if Farm == "Level" then
     ...
     elseif VerifyNPC("Awakened Ice Admiral") and getgenv().AutoRengoku then return
true
     elseif VerifyNPC("Darkbeard") and getgenv().AutoDarkbeard then return true
     elseif VerifyNPC("rip_indra True Form") and getgenv().AutoRipIndra then return
true
     ...
     elseif VerifyTool("Hallow Essence") and getgenv().AutoSoulReaper then return
true
     end
...
true
  ...
  elseif Farm == "Ectoplasm" then
     if getgenv().TeleportToFruit and FruitFind() then return true
     elseif getgenv().AutoCursedCaptain and VerifyNPC("Cursed Captain") then return
true
     end
  elseif Farm == "Hallow Boss" then
     if getgenv().AutoFarmBone and not VerifyNPC("Soul Reaper") and not
VerifyTool("Hallow Essence") then return true
     elseif getgenv().AutoRipIndra and VerifyNPC("rip_indra True Form") then
     end
  elseif Farm == "Raid" then
     if getgenv().TeleportToFruit and FruitFind() and not TimerG.Visible then return
true
     end
  elseif Farm == "Don Swan" then
     if getgenv().AutoFarm_Level and not VerifyNPC("Don Swan") then return true
     elseif getgenv().AutoFarmEctoplasm and not VerifyNPC("Don Swan") then return
true
...
true
  ...
  elseif Farm == "Cake Prince" then
     ...
     elseif not VerifyNPC("Cake Prince") and getgenv().TeleportToFruit and
FruitFind() then return true
     elseif not VerifyNPC("Dough King") and getgenv().TeleportToFruit and
FruitFind() then return true
     elseif VerifyNPC("Soul Reaper") and getgenv().AutoSoulReaper then return true
     elseif VerifyNPC("rip_indra True Form") and getgenv().AutoRipIndra then return
true
  ...
  elseif Farm == "Rip Indra" then
     if not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") and
getgenv().AutoFarm_Level then return true
     elseif not VerifyNPC("rip_indra True Form") and getgenv().RipIndraLegendaryHaki
and GetButton() then
     elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form")
and getgenv().AutoEliteHunter then return true
     elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form")
and getgenv().TeleportToFruit then return true
     elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form")
and getgenv().AutoFarmBone then return true
     elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form")
and getgenv().AutoCakePrince then return true
     elseif not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form")
and getgenv().AutoCakePrince then return true
     end
  end
...
      local Enemie = GetEnemies({Quest[3]})QuestVisible()
      ...
      elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
        ...
        local plrPP = Player.Character and Player.Character.PrimaryPart
        if QuestTween and QuestTween[1] and plrPP and (plrPP.Position -
QuestTween[1].p).Magnitude < 1200 then
...
local function AutoFarmMastery()
  local Quest = {CFrame.new(-16540, 56, 1051), false, NPCs = {"Isle Champion",
"Sun-kissed Warrior"}, "TikiQuest2", 2, true}
  local QuestTween = {
  ...
  if Sea1 then
    Quest = {CFrame.new(5258, 39, 4052), false, NPCs = {"Galley Captain"},
"FountainQuest", 2}
    QuestTween = {
  ...
  elseif Sea2 then
    Quest = {CFrame.new(-3056, 240, -10145), false, NPCs = {"Water Fighter"},
"ForgottenQuest", 2}
    QuestTween = {
...
     if not QuestVisible() and not VerifyQuest(Quest[3]) then
     ...
     elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
       local Health = Enemie and Enemie:FindFirstChild("Humanoid") and
Enemie.Humanoid.Health
       if Health and Health < Enemie.Humanoid.MaxHealth * SkillHealth / 100 then
         PlayerTP(Enemie.HumanoidRootPart.CFrame + Vector3.new(0,
getgenv().FarmDistance, 0))
         pcall(function()PlayerClick()ActiveHaki()EquipToolTip(Tool)BringNPC(Enemie,
true)end)
         if getgenv().AimBotSkill then
       ...
       elseif Health and Health >= Enemie.Humanoid.MaxHealth * SkillHealth / 100
then
         PlayerTP(Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos)
...
    for _,boat in pairs(Boats:GetChildren()) do
      if boat:FindFirstChild("Humanoid") and boat.Humanoid.Value > 0 then
        if boat:FindFirstChild("Owner") and boat.Owner.Value.Name == Player.Name
then
          Boat = boat
          return boat
        end
  ...
  local function GetFireflies()
     ...
     for _,fire in pairs(workspace:GetChildren()) do
       if fire.Name == "EmberTemplate" and fire:FindFirstChild("Part") then
         local plrPP = Player.Character and Player.Character.PrimaryPart
         if plrPP and (plrPP.Position - fire.Part.Position).Magnitude <= Nearest
then
  ...
  while getgenv().AutoKitsuneIsland do task.wait()
    ...
    if Boat then
      ...
      for _,part in pairs(Boat:GetDescendants()) do
        if part:IsA("BasePart") or part:IsA("MeshPart") and part.Name ~=
"VehicleSeat" then
          part.CanCollide = false
        end
      end
...
         elseif Stone then
      ...
      elseif AzureEmber then
         local plrPP = Player.Character and Player.Character.PrimaryPart
         if plrPP and block and (AzureEmber.Position - plrPP.Position).Magnitude <
100 then
    ...
    elseif not Boat then
      local BuyBoatDistance = (plrChar.PrimaryPart.Position - Vector3.new(-6123,
16, -2247)).Magnitude
      ...
      while getgenv().AutoFarmSea do task.wait()
        ...
        if BoatPP then
          if toPos1 then
            ...
            if (BoatPP.Position - Vector3.new(Distance, 21, 500)).Magnitude < 50
then
                toPos1, toPos2 = false, true
              end
            elseif toPos2 then
              TeleportPos = CFrame.new(Distance - 3000, 21, 500)
              if (BoatPP.Position - Vector3.new(Distance - 3000, 21, 500)).Magnitude
< 50 then
                toPos2, toPos3 = false, true
              end
            elseif toPos3 then
              TeleportPos = CFrame.new(Distance - 3000, 21, 2000)
              if (BoatPP.Position - Vector3.new(Distance - 3000, 21, 2000)).Magnitude
< 50 then
                toPos3, toPos4 = false, true
              end
            elseif toPos4 then
              TeleportPos = CFrame.new(Distance, 21, -1000)
              if (BoatPP.Position - Vector3.new(Distance, 21, -1000)).Magnitude < 50
then
               toPos4, toPos1 = false, true
             end
           end
        ...
        if plrH.Health < plrH.MaxHealth * getgenv().HealthPanic / 100 then
          ...
          repeat task.wait()
            if Boat and plrH then
              if not plrH.SeatPart or Boat:FindFirstChild("VehicleSeat") and
plrH.SeatPart ~= Boat.VehicleSeat then
                PlayerTP(Boat.VehicleSeat.CFrame)
                if Boat.PrimaryPart then
                  BoatTP(Boat, Boat.PrimaryPart.CFrame)
                end
              elseif Boat.PrimaryPart then
                local vector3 = Vector3.new(0,
math.clamp(Boat.PrimaryPart.Position.Y + 5, 0, 600), 0)
          ...
          or not plrChar
          or not plrH
          or plrH.Health > plrH.MaxHealth * math.clamp(getgenv().HealthPanic, 20,
75) / 100
    ...
    local function Verify(_,Kill)
      if Kill.Name == BoatName then
        if Kill:FindFirstChild("Health") and Kill.Health.Value > 0 then
          ...
          if BoatPP and KillPP and (BoatPP.Position - KillPP.Position).Magnitude <
Distance then
    ...
    local function Verify(_,Enemie)
      if Enemie.Name == EnemieName then
        ...
        if EnemieH and EnemieH.Health > 0 then
          ...
          if BoatPP and EnemiePP and (BoatPP.Position -
EnemiePP.Position).Magnitude < Distance then
...
16, -2247)).Magnitude
    ...
    elseif getgenv().Terrorshark and Terrorshark and
Terrorshark:FindFirstChild("HumanoidRootPart") then

      PlayerTP(Terrorshark.HumanoidRootPart.CFrame * CFrame.new(0, 40, 50))
      pcall(function()PlayerClick()ActiveHaki()EquipTool()plrChar.Humanoid.Sit =
false end)

      if Terrorshark:FindFirstChild("Humanoid") and Terrorshark.Humanoid.Health <=
0 then
        Terrorshark:Destroy()
      end

    elseif getgenv().Piranha and Piranha and
Piranha:FindFirstChild("HumanoidRootPart") then

      PlayerTP(Piranha.HumanoidRootPart.CFrame + getgenv().FarmPos)

pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(Piranha)plrChar.Humano
id.Sit = false end)
    elseif getgenv().FishCrewMember and FishCrewMember and
FishCrewMember:FindFirstChild("HumanoidRootPart") then

       PlayerTP(FishCrewMember.HumanoidRootPart.CFrame + getgenv().FarmPos)

pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(FishCrewMember)plrChar
.Humanoid.Sit = false end)

    elseif getgenv().Shark and Shark and Shark:FindFirstChild("HumanoidRootPart")
then
...
false end)
    ...
    elseif getgenv().PirateGrandBrigade and Boat1 and Boat1.PrimaryPart then
      ...
      if plrChar and plrChar.PrimaryPart and (plrChar.PrimaryPart.Position -
Boat1.PrimaryPart.Position).Magnitude < 60 then
        if getgenv().SeaAimBotSkill then
...
      EquipToolTip(RandomEquip)
      ...
      if plrChar and plrChar.PrimaryPart and (plrChar.PrimaryPart.Position -
Boat2.PrimaryPart.Position).Magnitude < 60 then
        if getgenv().SeaAimBotSkill then
    ...
    elseif getgenv().FishBoat and Boat3 and Boat3.PrimaryPart then
      ...
      if plrChar and plrChar.PrimaryPart and (plrChar.PrimaryPart.Position -
Boat3.PrimaryPart.Position).Magnitude < 60 then
        if getgenv().SeaAimBotSkill then
...
9e9):WaitForChild("CommF_", 9e9)
  ...
  while getgenv().AutoKenV2 do task.wait()
    ...
    elseif QuestProgress == 3 then
       if VerifyTool("") then
       elseif VerifyTool("Pineapple") and VerifyTool("Apple") and
VerifyTool("Banana") then
         local plrPP = Player.Character and Player.Character.PrimaryPart
         if plrPP and (plrPP.Position - Vector3.new(-12445, 332, -7676)).Magnitude <
5 then
        ...
        if plrPP then
          if PineappleS:FindFirstChild("Pineapple") and not VerifyTool("Pineapple")
then
...
            end
    ...
    elseif not BanditProgress then QuestVisible()
      ...
      if QuestActive.Visible and not string.find(QuestTitle.Text, "Forest Pirate")
and not string.find(QuestTitle.Text, "50") then
        FireRemote("AbandonQuest")
      end
      if not VerifyQuest("Forest Pirate") and not string.find(QuestTitle.Text,
"50") then
    ...
    elseif not BossProgress then
      local CaptainElephant = GetEnemies({"Captain Elephant"})QuestVisible()
       ...
       if QuestActive.Visible and not string.find(QuestTitle.Text, "Captain
Elephant") then
         FireRemote("AbandonQuest")
       end
       if not QuestActive.Visible and not string.find(QuestTitle.Text, "Captain
Elephant") then
         FireRemote("CitizenQuestProgress", "Citizen")
       elseif CaptainElephant and CaptainElephant:FindFirstChild("HumanoidRootPart")
then
...
and not string.find(QuestTitle.Text, "50") then
      ...
      if not QuestActive.Visible and not string.find(QuestTitle.Text, "Forest
Pirate") and not string.find(QuestTitle.Text, "50") then
        StartQuest1("CitizenQuest", 1)
        FireRemote("CitizenQuestProgress", "Citizen")
      elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
        PlayerTP(Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos)
...
Elephant") then
         FireRemote("CitizenQuestProgress", "Citizen")
       elseif CursedCaptain and CursedCaptain:FindFirstChild("HumanoidRootPart")
then
...
    local plrChar = Player and Player.Character and Player.Character.PrimaryPart
    ...
    for _,npc in pairs(Enemies1:GetChildren()) do
      if npc.Name == "Reborn Skeleton" or npc.Name == "Living Zombie" or npc.Name
== "Demonic Soul" or npc.Name == "Posessed Mummy" then
        if plrChar and npc and npc:FindFirstChild("HumanoidRootPart") and
(plrChar.Position - npc.HumanoidRootPart.Position).Magnitude <= Distance then
          Distance = (plrChar.Position - npc.HumanoidRootPart.Position).Magnitude
          NPC = npc
  ...
  task.spawn(function()
    while getgenv().AutoCursedDualKatana do task.wait()
      if FarmMastery then
        ...
        local Enemie =
GetMasteryEnemies()EquipToolName("Yama")EquipToolName("Tushita")
...
    end
  ...
  while getgenv().AutoCursedDualKatana do task.wait()
    ...
    elseif EvilProgress == 1 then
      ...
      TushitaQuest_1, TushitaQuest_2, TushitaQuest_3 = false, false, false
      Remote:InvokeServer("CDKQuest", "Progress",
"Evil")Remote:InvokeServer("CDKQuest", "StartTrial", "Evil")
...
"Evil")Remote:InvokeServer("CDKQuest", "StartTrial", "Evil")
    elseif GoodProgress == 1 then
      ...
      Remote:InvokeServer("CDKQuest", "Progress",
"Good")Remote:InvokeServer("CDKQuest", "StartTrial", "Good")
    ...
    for _,npc in pairs(Enemies:GetChildren()) do
      ...
      and (npc.PrimaryPart.Position - plrPP.Position).Magnitude < 1000 then
        ...
        if npc:FindFirstChild("HumanoidRootPart") then
                        ...
                        if npc:FindFirstChild("Humanoid") and
npc.Humanoid:FindFirstChild("Animator") then
                              npc.Humanoid.Animator:Destroy()
                        end
                        if npc:FindFirstChild("Humanoid") and npc.Humanoid.Health
<= 0 then
                          npc:Destroy()
                        end
                        sethiddenproperty(Player, "SimulationRadius", math.huge)
  ...
  while getgenv().AutoSoulGuitar do task.wait()
    ...
    if plrPP and (plrPP.Position - Vector3.new(-10129, 154, 5928)).Magnitude < 5
and BringZombie() > 5 then
      pcall(function()PlayerClick()ActiveHaki()EquipTool()end)
    end
  end
end
  ...
  while getgenv().AutoUnlockSaber do task.wait()
    if PlayerLevel.Value > 200 then
      ...
      elseif plrChar and VerifyTool("Relic") then
        ...
        EquipToolName("Relic")
      elseif SickManProgress == 1 and RichSonProgress == 0 and not
workspace.Map.Desert.Burn.Part.CanCollide then
      ...
      elseif plrChar:FindFirstChild("Cup") and not
plrChar["Cup"].Handle:FindFirstChild("TouchInterest") then
        FireRemote("ProQuestProgress", "SickMan")
      elseif plrChar:FindFirstChild("Cup") and
plrChar["Cup"].Handle:FindFirstChild("TouchInterest") then
        PlayerTP(CFrame.new(1393, 37, -1324, -0.408640295, 0, -0.912695527, 0, 1,
0, 0.912695527, 0, -0.408640295))
...
        if Plates then
          ...
          local Plate5   =   Plates:FindFirstChild("Plate5")
           ...
           if Plate1 and Plate1:FindFirstChild("Button") and
Plate1.Button.BrickColor ~= BrickColor.new("Camo") then
             PlayerTP(Plate1.Button.CFrame)
           elseif Plate2 and Plate2:FindFirstChild("Button") and
Plate2.Button.BrickColor ~= BrickColor.new("Camo") then
             PlayerTP(Plate2.Button.CFrame)
           elseif Plate3 and Plate3:FindFirstChild("Button") and
Plate3.Button.BrickColor ~= BrickColor.new("Camo") then
             PlayerTP(Plate3.Button.CFrame)
           elseif Plate4 and Plate4:FindFirstChild("Button") and
Plate4.Button.BrickColor ~= BrickColor.new("Camo") then
             PlayerTP(Plate4.Button.CFrame)
           elseif Plate5 and Plate5:FindFirstChild("Button") and
Plate5.Button.BrickColor ~= BrickColor.new("Camo") then
             PlayerTP(Plate5.Button.CFrame)
           end
         end
       elseif plrRP and Plates:FindFirstChild("Door") and not Plates.Door.CanCollide
then
...
local function AutoFarmEctoplasm()
  while getgenv().AutoFarmEctoplasm do task.wait()
    if Configure("Ectoplasm") then
      ...
      local Enemie = GetEnemies({"Ship Deckhand", "Ship Engineer", "Ship Steward",
"Ship Officer"})
    ...
    while getgenv().AutoFarmMaterial do task.wait()
      ...
      if Sea1 then
        if Material == "Angel Wings" then
          EnemiesList, NPCPos = {"Royal Soldier", "Royal Squad"}, CFrame.new(-7742,
5634, -1564)
       ...
       elseif Sea3 then
         ...
         elseif Material == "Fish Tail" then
           EnemiesList, NPCPos = {"Fishman Captain", "Fishman Raider"}, CFrame.new(-
10679, 398, -8975)
         ...
         elseif Material == "Mini Tusk" then
           EnemiesList, NPCPos = {"Mythological Pirate"}, CFrame.new(-13510, 584, -
6986)
         elseif Material == "Conjured Cocoa" then
           EnemiesList, NPCPos = {"Cocoa Warrior", "Chocolate Bar Battler"},
...
    if Configure("Bone") then
      ...
      local Enemie = GetEnemies({"Reborn Skeleton", "Living Zombie", "Demonic
Soul", "Posessed Mummy"})
      if Enemie then
         PlayerTP(Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos)
         pcall(function()PlayerClick()ActiveHaki()EquipTool()BringNPC(Enemie,
      ...
      for _,npc in pairs(Enemies:GetChildren()) do
         if npc.Name ~= "rip_indra True Form" then
          ...
          if npcH and npcH.Health > 0 then
            if npc and npc.PrimaryPart and (npc.PrimaryPart.Position -
Vector3.new(-5556, 314, -2988)).Magnitude < 700 then
              Enemie = npc
            end
          end
        end
...
local function AutoSecondSea()
  ...
  while getgenv().AutoSecondSea do task.wait()
    if PlayerLevel.Value < 700 then
      ...
      local plrChar = Player and Player.Character
      local plrPP = Player and Player.Character and Player.Character.PrimaryPart
      local DoorArgs = IceMap:FindFirstChild("Door") and IceMap.Door.CanCollide and
IceMap.Door.Transparency < 1

       if not VerifyTool("Key") and DoorArgs then
         PlayerTP(CFrame.new(4852, 6, 719))
         if plrPP and (plrPP.Position - Vector3.new(4852, 6, 719)).Magnitude < 5
then
      ...
      elseif VerifyNPC("Ice Admiral") and IceMap:FindFirstChild("Door") and not
IceMap.Door.CanCollide and IceMap.Door.Transparency > 0 then
...
Player.Character.PrimaryPart
          if plrChar and (plrChar.Position - Vector3.new(-1926, 13,
1738)).Magnitude < 5 then
  ...
  while getgenv().AutoBartiloQuest do task.wait()
    if PlayerLevel.Value >= 850 then
      ...
      if BartiloQuestLevel == 0 then
        ...
        if QuestActive.Visible and not string.find(QuestTitle.Text, "Swan Pirate")
and not string.find(QuestTitle.Text, "50") then
          FireRemote("AbandonQuest")
        end

        if not QuestActive.Visible and not string.find(QuestTitle.Text, "Swan
Pirate") and not string.find(QuestTitle.Text, "50") then
          ...
          TweenNPCSpawn({CFrame.new(778, 110, 1129), CFrame.new(1018, 110, 1128),
CFrame.new(1020, 110, 1366), CFrame.new(1016, 110, 1115)}, "Swan Pirate")
        end
      elseif BartiloQuestLevel == 1 then
       ...
       elseif BartiloQuestLevel == 2 then
         local Plates =
game:GetService("Workspace").Map.Dressrosa:FindFirstChild("BartiloPlates")
         if Plates and Plates:FindFirstChild("Plate1") and Plates.Plate1.Color.G ~=
1 then
           PlayerTP(Plates.Plate1.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate2") and Plates.Plate2.Color.G
~= 1 then
           PlayerTP(Plates.Plate2.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate3") and Plates.Plate3.Color.G
~= 1 then
           PlayerTP(Plates.Plate3.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate4") and Plates.Plate4.Color.G
~= 1 then
           PlayerTP(Plates.Plate4.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate5") and Plates.Plate5.Color.G
~= 1 then
           PlayerTP(Plates.Plate5.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate6") and Plates.Plate6.Color.G
~= 1 then
           PlayerTP(Plates.Plate6.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate7") and Plates.Plate7.Color.G
~= 1 then
           PlayerTP(Plates.Plate7.CFrame)
         elseif Plates and Plates:FindFirstChild("Plate8") and Plates.Plate8.Color.G
~= 1 then
    ...
    if Configure("Cake Prince") then
      ...
      elseif VerifyNPC("Cake Prince") then
        ...
        if not getgenv().AutoFarm_Level and not getgenv().AutoFarmBone then
           local Enemie = GetEnemies({"Head Baker", "Baking Staff", "Cake Guard",
"Cookie Crafter"})
...
true)end)
      ...
      elseif VerifyNPC("Urban") and not VerifyTool("God's Chalice") and not
VerifyTool("Sweet Chalice")
      or VerifyNPC("Deandre") and not VerifyTool("God's Chalice") and not
VerifyTool("Sweet Chalice")
      or VerifyNPC("Diablo") and not VerifyTool("God's Chalice") and not
VerifyTool("Sweet Chalice") then
        local NPC = "EliteHunterVerify"QuestVisible()
...
local function KillAllBosses()
  ...
  while getgenv().KillAllBosses do task.wait()
    if SelectedBoss then
      ...
      if Boss and GetQuest and getgenv().TakeQuestBoss and not QuestVisible() and
not VerifyQuest(Boss.Name) then
...
local function AutoFarmBossSelected()
  local GetQuest, GetQuestPos, Quest, NotBossPos, BossName = false, nil, nil, nil,
getgenv().BossSelected or ""
  local QuestActive = Player.PlayerGui.Main.Quest
  ...
  while getgenv().AutoFarmBossSelected do task.wait()
    ...
    if plrChar then
      ...
      if VerifyNPC(BossName) then
        ...
        if QuestActive.Visible and not
string.find(QuestActive.Container.QuestTitle.Title.Text, BossName) then
          FireRemote("AbandonQuest")
        end
        if GetQuest and getgenv().TakeQuestBoss and not QuestActive.Visible and not
string.find(QuestActive.Container.QuestTitle.Title.Text, BossName) then
          StartQuest1(Quest, QuestNun or 3)
  ...
  while getgenv().AutoSoulReaper do task.wait()
    if Configure("Hallow Boss") then
      ...
      elseif VerifyTool("Hallow Essence") then
        ...
        pcall(function()PlayerTP(workspace.Map["Haunted
Castle"].Summoner.Detection.CFrame)end)
      else
...
EnemiePP.Position).Magnitude <= Distance then
  ...
  while getgenv().AutoFarmNearest do task.wait()
    if getgenv().AutoFarm_Level or getgenv().AutoRipIndra or
...
Player.Character.PrimaryPart
        ...
        if plrChar and (plrChar.Position - Vector3.new(-2777, 73, -3570)).Magnitude
< 5 then
...
        local plrBag = Player:FindFirstChild("Backpack")
        ...
        if not plrBag:FindFirstChild("Flower 1") and
workspace:FindFirstChild("Flower1") and workspace["Flower1"].Transparency ~= 1 then
          PlayerTP(workspace["Flower1"].CFrame)
        elseif not plrBag:FindFirstChild("Flower 2") and
workspace:FindFirstChild("Flower2") and workspace["Flower2"].Transparency ~= 1 then
...
CFrame.new(1020, 110, 1366), CFrame.new(1016, 110, 1115)}, "Swan Pirate")
        ...
        elseif not plrBag:FindFirstChild("Flower 1") and
workspace:FindFirstChild("Flower1") then
          PlayerTP(workspace["Flower1"].CFrame)
        elseif not plrBag:FindFirstChild("Flower 2") and
workspace:FindFirstChild("Flower2") then
          PlayerTP(workspace["Flower2"].CFrame)
        end
      elseif QuestProgress == 2 then
        local plrChar = Player and Player.Character and
...
      elseif VerifyNPC("Awakened Ice Admiral") then
        ...
        if AwakenedIceAdmiral and
AwakenedIceAdmiral:FindFirstChild("HumanoidRootPart") then
...
if Sea3 then
  ...
  AutoSea:AddToggle("Toggle", {Title = "Auto Kitsune Island", Callback =
function(Value)
    getgenv().AutoKitsuneIsland = Value;AutoKitsuneIsland()
  end})
  AutoSea:AddToggle("Toggle", {Title = "Auto Trade Azure Ember",Callback =
function(Value)
...
  })
  ...
  AutoSea:AddToggle("Toggle", {Title = "Panic Mode", Default = true,Callback =
function(Value)
     getgenv().PanicMode = Value
  end})
  AutoSea:AddSection("Farm Select")
  AutoSea:AddParagraph({Title = "Fish"})
  AutoSea:AddToggle("Toggle", {Title = "Terrorshark", Default = true,Callback =
function(Value)
     getgenv().Terrorshark = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Piranha", Default = true,Callback =
function(Value)
     getgenv().Piranha = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Fish Crew Member", Default = true,Callback
= function(Value)
     getgenv().FishCrewMember = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Shark", Default = true,Callback =
function(Value)
     getgenv().Shark = Value
  end})
  AutoSea:AddParagraph({Title = "Boats"})
  AutoSea:AddToggle("Toggle", {Title = "Pirate Brigade", Default = true,Callback =
function(Value)
     getgenv().PirateBrigade = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Pirate Grand Brigade", Default =
true,Callback = function(Value)
     getgenv().PirateGrandBrigade = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Fish Boat", Default = true,Callback =
function(Value)
     getgenv().FishBoat = Value
  end})
  --[[AddTextLabel(AutoSea, {"Sea Beast"})
  AutoSea:AddToggle("Toggle", {Title = "Sea Beast",Default = true,Callback =
function(Value)
     getgenv().SeaBeast = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Triple Sea Beast",Default = true,Callback =
function(Value)
     getgenv().SeaBeastTriple = Value
  end})]]
  AutoSea:AddSection("Skill")
  AutoSea:AddToggle("Toggle", {Title = "AimBot Skill Enemie", Default =
true,Callback = function(Value)
     getgenv().SeaAimBotSkill = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Skill Z", Default = true,Callback =
function(Value)
     getgenv().SeaSkillZ = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Skill X", Default = true,Callback =
function(Value)
     getgenv().SeaSkillX = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Skill C", Default = true,Callback =
function(Value)
     getgenv().SeaSkillC = Value
  end})
  AutoSea:AddToggle("Toggle", {Title = "Skill V", Default = true,Callback =
function(Value)
  ...
  AutoSea:AddToggle("Toggle", {Title = "Teleport To Shark Hunter",Callback =
function(Value)
  ...
  AutoSea:AddToggle("Toggle", {Title = "Teleport To Beast Hunter",Callback =
function(Value)
     getgenv().NPCtween = Value
     task.spawn(function()
       while getgenv().NPCtween do task.wait()
...
if Sea3 and Player.UserId == 2764978820 then
  ...
  MirageTab:AddToggle("Toggle", {Title = "Auto Pull Lever", Callback =
function(Value)

  end})

  MirageTab:AddToggle("Toggle", {Title = "Auto Stone Puzzle", Callback =
function(Value)
  ...
  MirageTab:AddToggle("Toggle", {Title = "Auto Find Mirage", Callback =
function(Value)

  end})

  MirageTab:AddToggle("Toggle", {Title = "Auto Gear Puzzle", Callback =
function(Value)
  ...
  MirageTab:AddToggle("Toggle", {Title = "Auto Finish Trial", Callback =
function(Value)
...
if PlayerLevel.Value >= 2550 and Sea3 then
  MainFarm:AddToggle("Toggle", {Title = "Auto Farm Candy", Callback =
function(Value)
...
Castle"].RaidSummon2.Button.Main.ClickDetector)
  ...
  FruitAndRaid:AddToggle("Toggle", {Title = "Auto Buy Chip", Callback =
function(Value)
  ...
  if Player.UserId ~= 2764978820 then
    task.spawn(function()
      while task.wait(1) do
         LabelElit3:Set("Elite Hunter progress : " .. FireRemote("EliteHunter",
"Progress"))
      end
    end)
  end
...
fireclickdetector(workspace.Map.Waterfall.SealedKatana.Handle.ClickDetector)
  ...
  local LabelRipIndra = QuestsTabs:AddParagraph({Title = "Rip Indra Stats : not
Spawn"})
...
VerifyNPC("Urban") then
  ...
  QuestsTabs:AddToggle("Toggle", {Title = "Auto Cake Prince", Callback =
function(Value)
    getgenv().AutoCakePrince = Value
    AutoCakePrince()
  end})

  QuestsTabs:AddToggle("Toggle", {Title = "Auto Dough King", Callback =
function(Value)
...
  local ActiveButtonToggle = QuestsTabs:AddToggle("Toggle", {Title = "Auto Active
Button Haki Color", Callback = function(Value)
...
getgenv().AutoCakePrince then
  ...
  QuestsTabs:AddToggle("Toggle", {Title = "Auto Rip Indra", Callback =
function(Value)
...
Teleport:AddToggle("Toggle", {Title = "Teleport To Island",Callback =
function(Value)
...
Misc:AddTextBox({Title = "Server ID",Default = "",PlaceholderText = "Server
...
Misc:AddSlider("Slider", {Title = "Farm Distance",Min = 5,Max = 50,Default =
20,Rounding = 1,Callback = function(Value)
  getgenv().FarmPos = Vector3.new(0, Value or 15, Value or
10)getgenv().FarmDistance = Value
end})

Misc:AddSlider("Slider", {Title = "Tween Speed",Min = 50,Max = 300,Default =
170,Rounding = 5,Callback = function(Value)
  getgenv().TweenSpeed = Value
end})
Misc:AddSlider("Slider", {Title = "Bring Mobs Distance",Min = 50,Max = 500,Default
= 250,Rounding = 10,Callback = function(Value)
  getgenv().BringMobsDistance = Value or 250
end})
Misc:AddSlider("Slider", {Title = "Auto Click Delay",Min = 0.01,Max = 1,Default =
0.18,Rounding = 0.01,Callback = function(Value)
...
Misc:AddToggle("Toggle", {Title = "Increase Attack Distance", true, Callback =
function(Value)
...
Misc:AddToggle("Toggle", {Title = "Remove Notifications", Callback =
function(Value)
    ...
    task.spawn(function()
      local Map = workspace:WaitForChild("Map", 9e9)
        ...
        while getgenv().WalkOnWater do task.wait(0.1)
          Map:WaitForChild("WaterBase-Plane", 9e9).Size = Vector3.new(1000, 113,
1000)
        end
...
Shop:AddButton({Title = Buy 2x EXP (15 mins.) < 50 Candies >",
function()FireRemote("Candies", "Check")FireRemote("Candies", "Buy", 1, 1)end})
Shop:AddButton({Title = Stats Refund < 75 Candies >",
function()FireRemote("Candies", "Check")FireRemote("Candies", "Buy", 1, 2)end})
Shop:AddButton({Title = Race Reroll < 100 Candies >",
function()FireRemote("Candies", "Check")FireRemote("Candies", "Buy", 1, 3)end})
Shop:AddSection("")
Shop:AddButton({Title = Buy 200 Frags < 50 Candies >",
function()FireRemote("Candies", "Check")FireRemote("Candies", "Buy", 2, 2)end})
Shop:AddButton({Title = Buy 500 Frags < 100 Candies >",
function()FireRemote("Candies", "Check")FireRemote("Candies", "Buy", 2, 2)end})
Shop:AddSection("Bones")
Shop:AddButton({Title = Buy Surprise < 50 Bones >", function()FireRemote("Bones",
"Buy", 1, 1)end})
Shop:AddButton({Title = Stats Refund < 150 Bones >", function()FireRemote("Bones",
"Buy", 1, 2)end})
Shop:AddButton({Title = Race Reroll < 300 Bones >", function()FireRemote("Bones",
"Buy", 1, 3)end})]]
...
Shop:AddButton({Title = "Buy Fishman Karate",
function()FireRemote("BuyFishmanKarate")end})
Shop:AddButton({Title = "Buy Dragon Claw", function()
FireRemote("BlackbeardReward","DragonClaw","1")FireRemote("BlackbeardReward","Drago
nClaw","2")end})
Shop:AddButton({Title = "Buy Superhuman",
function()FireRemote("BuySuperhuman")end})
Shop:AddButton({Title = "Buy Death Step", function()FireRemote("BuyDeathStep")end})
Shop:AddButton({Title = "Buy Sharkman Karate",
function()FireRemote("BuySharkmanKarate")end})
Shop:AddButton({Title = "Buy Electric Claw",
function()FireRemote("BuyElectricClaw")end})
Shop:AddButton({Title = "Buy Dragon Talon",
function()FireRemote("BuyDragonTalon")end})
Shop:AddButton({Title = "Buy GodHuman", function()FireRemote("BuyGodhuman")end})
Shop:AddButton({Title = "Buy Sanguine Art",
function()FireRemote("BuySanguineArt")end})
...
Shop:AddButton({Title = "Buy Katana", function()FireRemote("BuyItem",
"Katana")end})
Shop:AddButton({Title = "Buy Cutlass", function()FireRemote("BuyItem",
"Cutlass")end})
Shop:AddButton({Title = "Buy Dual Katana", function()FireRemote("BuyItem", "Dual
Katana")end})
Shop:AddButton({Title = "Buy Iron Mace", function()FireRemote("BuyItem", "Iron
Mace")end})
Shop:AddButton({Title = "Buy Triple Katana", function()FireRemote("BuyItem",
"Triple Katana")end})
Shop:AddButton({Title = "Buy Pipe", function()FireRemote("BuyItem", "Pipe")end})
Shop:AddButton({Title = "Buy Dual-Headed Blade", function()FireRemote("BuyItem",
"Dual-Headed Blade")end})
Shop:AddButton({Title = "Buy Soul Cane", function()FireRemote("BuyItem", "Soul
Cane")end})
Shop:AddButton({Title = "Buy Bisento", function()FireRemote("BuyItem",
"Bisento")end})
Shop:AddSection("Gun")
Shop:AddButton({Title = "Buy Musket", function()FireRemote("BuyItem",
"Musket")end})
Shop:AddButton({Title = "Buy Slingshot", function()FireRemote("BuyItem",
"Slingshot")end})
Shop:AddButton({Title = "Buy Flintlock", function()FireRemote("BuyItem",
"Flintlock")end})
Shop:AddButton({Title = "Buy Refined Slingshot", function()FireRemote("BuyItem",
"Refined Slingshot")end})
Shop:AddButton({Title = "Buy Refined Flintlock", function()FireRemote("BuyItem",
"Refined Flintlock")end})
Shop:AddButton({Title = "Buy Cannon", function()FireRemote("BuyItem",
"Cannon")end})
...
Shop:AddButton({Title = "Buy Black Cape", function()FireRemote("BuyItem", "Black
Cape")end})
Shop:AddButton({Title = "Buy Swordsman Hat", function()FireRemote("BuyItem",
"Swordsman Hat")end})
Shop:AddButton({Title = "Buy Tomoe Ring", function()FireRemote("BuyItem", "Tomoe
Ring")end})
Shop:AddSection("Race")
Shop:AddButton({Title = "Ghoul Race", function()FireRemote("Ectoplasm", "Change",
4)end})
Shop:AddButton({Title = "Cyborg Race", function()FireRemote("CyborgTrainer",
"Buy")end})
...
Visual:AddSlider("Slider", {Title = "Nofication Time",Max = 120, Min = 5, Rounding
= 1, Default = 15, Callback = function(Value)
  NotifiTime = Value
end})
...
Visual:AddTextBox({Title = "Fake Health",Default = "",PlaceholderText =
...
Visual:AddTextBox({Title = "Fake Money",Default = "",PlaceholderText =
"Money",Callback = function(Value)
  Player.Data.Beli.Value = Value
end})
Visual:AddTextBox({Title = "Fake Fragments",Default = "",PlaceholderText =
