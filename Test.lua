...
local redzlib = loadstring(game:HttpGet("https://pastefy.app/ZE9qbrr2/raw"))()
...
Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://123060196429580", BackgroundTransparency = 0 },
...
AttackDropdown = Tabs.Main:AddDropdown({
    ...
    Options = {"Normal Attack","Fast Attack","Super Fast Attack","NEJI Attack"},
    Default = "Fast Attack",
    Callback = function(Value)
...
Tabs.Main:AddSection({"Easter Events"})
Tabs.Main:AddButton({
    Name = "Open Easter Shop",
    Description = "",
    Callback = function()
        -- Sử dụng pcall để tránh lỗi văng script nếu game chưa load kịp Module
        local success, EventShop = pcall(function()
            return require(game:GetService("ReplicatedStorage"):WaitForChild("Controllers"):WaitForChild("UI"):WaitForChild("EventShop"))
        end)

        if success and EventShop and EventShop.Open then
            EventShop:Open("Easter2026")
        else
            -- Thông báo lỗi nhẹ nhàng nếu không tìm thấy
            warn("Không thể mở Easter Shop. Có thể game đã cập nhật hoặc sai đường dẫn Module.")
        end
    end
})
local RS = game:GetService("ReplicatedStorage")
local CS = game:GetService("CollectionService")
local Net = require(RS.Controllers.UI.EasterCodex.EasterNetwork)
local Remote = RS.Modules.Net:WaitForChild("RE/EasterServiceRE")
local plr = game.Players.LocalPlayer

FarmEggToggle = Tabs.Main:AddToggle({
    Name = "Auto Collect Egg Nearest", 
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.FarmEgg = Value
...
task.spawn(function()
    while task.wait(0.2) do
        -- Kiểm tra điều kiện: Nếu tắt Toggle thì bỏ qua vòng lặp
        if not _G.FarmEgg then continue end

        local root = hrp()
        if not root then continue end

        for _, obj in pairs(CS:GetTagged("EasterEgg26")) do
            local uid = obj:GetAttribute("UID")
            local cf = obj:GetAttribute("CFrame")
            local state = Net.Streaming[uid] and Net.Streaming[uid].State

            if uid and cf and state ~= "Collected" then
                local pos = cf.Position + Vector3.new(0, 2, 0) -- offset nhẹ tránh kẹt

                root.CFrame = CFrame.new(pos)

                print("COLLECT:", uid, state)

                Remote:FireServer("CollectEgg", {
                    UID = uid,
                    Additional = {}
                })
            end
...
Tabs.Main:AddToggle({
    Name = "Auto Collect Egg Mythical Fast",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.InstantPrompt = Value
        
        -- Ngay khi bật Toggle, quét dọn 1 lần các vật thể đang có sẵn ở gần
        if Value then
            for _, prompt in pairs(workspace:GetDescendants()) do
                if prompt:IsA("ProximityPrompt") then
                    -- Lưu lại thời gian gốc lỡ bạn muốn tắt Toggle
            ...
            -- Khi tắt Toggle, trả lại thời gian chờ bình thường cho game
            for _, prompt in pairs(workspace:GetDescendants()) do
                if prompt:IsA("ProximityPrompt") and prompt:GetAttribute("OriginalHold") then
...
workspace.DescendantAdded:Connect(function(descendant)
    if _G.InstantPrompt and descendant:IsA("ProximityPrompt") then
        if not descendant:GetAttribute("OriginalHold") then
            descendant:SetAttribute("OriginalHold", descendant.HoldDuration)
        end
...
Name = "Auto Collect Berry + Hop Sever", 
Description = "", 
Default = false,
Callback = function(Value)
  _G.AutoBerryH = Value
...
Q = Tabs.Mirage:AddToggle({
Name = "Auto Farm Materials Soul Guitar", 
Description = "", 
Default = false,
Callback = function(Value)
...
Q = Tabs.SeaEvent:AddToggle({
Name = "Auto Sail", 
Description = "Ngồi Vào Chỗ Rồi Bật", 
Default = false,
Callback = function(Value)
  _G.SailBoats = Value
end})
...
Tabs.Misc:AddButton({
Name = "Hop Sever Lowest Ping", 
Description = "Thma Gia Máy Chủ Ping Thấp",
Callback = function()
local HTTPService = game:GetService("HttpService")
local TeleportService = game:GetService("TeleportService")
...
Description = "Mở Khoá Tầm Nhìn Giới Hạn", 
Default = false,
Callback = function(Value)
  bright = Value
  if Value == true then
