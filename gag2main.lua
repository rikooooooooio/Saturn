-- ===========================================================
-- 🪐 SATURN HUB - Grow a Garden 2 (Saturn UI / PiHub) – COMPLETO
-- ===========================================================
...
local S = {
    ...
    fpsBoost = false, hidePlayers = false,
    shovelWhenReady = false, singleSpot = false,
...
-- ===========================================================
-- FUNÇÕES PRINCIPAIS DO FARM (mantidas iguais, com ajustes)
-- ===========================================================
...
local function stepPlant()
    ...
    -- Single-spot mode: apenas o primeiro ponto
    if S.singleSpot and #grid > 0 then
        grid = { grid[1] }
    end
...
local function stepHarvest()
    ...
    -- Shovel when ready: se houver plantas maduras, usa a pá antes de colher
    if S.shovelWhenReady then
        ...
        if shovel then
            ...
            task.wait(1.5) -- aguarda a animação
        end
    end
...
-- ===========================================================
-- PERFORMANCE & VISUAL (mantido + hide players)
-- ===========================================================
...
-- ===========================================================
-- UI SATURN (PiHub) + Multi-dropdown customizado
-- ===========================================================
...
local Tabs = {
    ...
    Stats = Window:Tab("Stats", "rbxassetid://10709770317"),
    Settings = Window:Tab("Settings", "rbxassetid://10734883986")
}

-- Função para criar um multi-dropdown (baseado em toggles)
local function CreateMultiDropdown(tab, title, options, initialSelected, callback)
    local selected = {}
    if initialSelected then
        for _, name in ipairs(initialSelected) do selected[name] = true end
    end
    local isOpen = false
    local mainToggle = tab:Toggle(title, false, "Click to select multiple", function(state)
        isOpen = state
        if optionFrame then optionFrame.Visible = state end
    end)
    local optionFrame = Instance.new("Frame")
    optionFrame.Name = "MultiDropdown_" .. title
    optionFrame.Parent = mainToggle.Instance.Parent -- mesmo pai do toggle
    optionFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    optionFrame.BackgroundTransparency = 0.4
    optionFrame.Size = UDim2.new(1, -20, 0, 200)
    optionFrame.Visible = false
    local corner = Instance.new("UICorner", optionFrame); corner.CornerRadius = UDim.new(0, 8)
    ...
    for _, opt in ipairs(options) do
        local toggle = tab:Toggle(opt, selected[opt] == true, "", function(state)
            if state then selected[opt] = true else selected[opt] = nil end
            local list = {}
            for name in pairs(selected) do table.insert(list, name) end
    ...
    local function refreshVisual()
        for _, toggle in ipairs(toggles) do
            -- Não há método público para setar o estado do toggle; precisamos recriar. Mas para simplificar,
            -- o estado já é gerenciado pelo callback.
        end
    end
    return {
        Get = function()
        ...
        Set = function(list)
            selected = {}
            for _, name in ipairs(list) do selected[name] = true end
            refreshVisual()
            if callback then callback(list) end
        end,
        Destroy = function()
            optionFrame:Destroy()
            for _, toggle in ipairs(toggles) do toggle:Destroy() end
            mainToggle:Destroy()
        end
    }
end
...
local buySeedsMulti = CreateMultiDropdown(Tabs.Farm, "Seeds to buy", SEED_NAMES, {}, function(list)
    pickMulti(list, S.buySeeds)
end)
...
Tabs.Farm:Toggle("Single-Spot Mode", false, "Plant everything in one spot", function(v) S.singleSpot = v end)
Tabs.Farm:Toggle("Shovel When Plants Ready", false, "Use shovel before harvesting", function(v) S.shovelWhenReady = v end)
...
local petSellMulti = CreateMultiDropdown(Tabs.Pets, "Pets to sell", ownedPetNames(), {}, function(list)
    pickMulti(list, S.sellPets)
end)
...
local gearMulti = CreateMultiDropdown(Tabs.Shop, "Gear to buy", GEAR_NAMES, {}, function(list)
    pickMulti(list, S.gearBuy)
end)
...
Tabs.Settings:Toggle("Hide Other Players", false, "Makes other players invisible", function(v)
    S.hidePlayers = v
    applyHidePlayers(v)
end)
...
local lblMoney = Tabs.Stats:Label("Money Earned: 0")
local lblHarvest = Tabs.Stats:Label("Plants Harvested: 0")
local lblPlanted = Tabs.Stats:Label("Seeds Planted: 0")
local lblTime = Tabs.Stats:Label("Time Played: 0s")
Tabs.Stats:Seperator("Inventory")
local lblSeeds = Tabs.Stats:Label("Total Seeds: 0")
local lblPlants = Tabs.Stats:Label("Total Plants: 0")
Tabs.Stats:Seperator("Most Profitable")
local lblProf = Tabs.Stats:Label("Most Profitable: Dragon Fruit")

task.spawn(function()
    while not S.killed do
        task.wait(1)
        local elapsed = math.floor(os.clock() - Stats.startAt)
        pcall(function() lblMoney:Set("Money Earned: " .. fmt(Stats.earned)) end)
        pcall(function() lblHarvest:Set("Plants Harvested: " .. Stats.harvested) end)
        pcall(function() lblPlanted:Set("Seeds Planted: " .. Stats.planted) end)
        pcall(function() lblTime:Set("Time Played: " .. hms(elapsed)) end)
        local sd, pl = 0, 0
        for _, t in ipairs(Backpack:GetChildren()) do
            if t:IsA("Tool") then
                if t:GetAttribute("SeedTool") then sd = sd + 1 end
                if t:GetAttribute("Fruit") then pl = pl + 1 end
            end
        end
        pcall(function() lblSeeds:Set("Total Seeds: " .. sd) end)
        pcall(function() lblPlants:Set("Total Plants: " .. pl) end)
        -- Most profitable placeholder (pode ser melhorado)
        pcall(function() lblProf:Set("Most Profitable: Dragon Fruit") end)
    end
end)
