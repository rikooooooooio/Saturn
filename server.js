...
const apiLimiter = rateLimit({ windowMs: 1 * 60 * 1000, max: 200, message: { error: 'Muitas requisições.' } });
const loaderLimiter = rateLimit({ windowMs: 1 * 60 * 1000, max: 100, message: 'Rate limit exceeded.' });
...
// Obter script completo (conteúdo em texto puro)
app.get('/api/scripts/:id', authMiddleware, (req, res) => {
  ...
  // Retorna o conteúdo como está (sem criptografia)
  res.json(script);
});
...
app.post('/api/scripts', authMiddleware, (req, res) => {
  ...
  const script = {
    id: uuidv4(),
    name: name.trim(),
    content,                    // texto puro, sem criptografia
...
app.put('/api/scripts/:id', authMiddleware, (req, res) => {
  ...
  if (content !== undefined) script.content = content;   // texto puro
...
// Upload em massa (.txt) — sem criptografia
...
app.post('/api/scripts/:id/changelog', authMiddleware, async (req, res) => {
  ...
  res.json({ success: true, message: 'Changelog enviado!' });
});
...
app.get('/status', (req, res) => {
  ...
  const total = DB.scripts.length;
  const exec = DB.scripts.reduce((a, s) => a + (s.executions || 0), 0);
  res.send(`<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8"><title>Saturn Status</title><style>body{background:#0a0a0a;color:#fff;font-family:Inter,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}.card{background:#111;border:1px solid #1f1f1f;border-radius:16px;padding:2.5rem;text-align:center;max-width:400px;width:90%}h1{margin-bottom:.5rem}p{color:#a0a0a0}.stat{font-size:2rem;font-weight:800;margin:.5rem 0}.dot{display:inline-block;width:10px;height:10px;background:#10b981;border-radius:50%;margin-right:.5rem}</style></head><body><div class="card"><h1>Saturn</h1><p>Sistema operacional</p><div class="stat"><span class="dot"></span>${online} / ${total} scripts online</div><p>${exec} execuções totais</p></div></body></html>`);
});
